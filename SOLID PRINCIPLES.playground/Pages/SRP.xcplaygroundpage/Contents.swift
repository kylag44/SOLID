 ...
 * When a class takes on more than one responsibility those responsibilities may become entangled (eg. calling each other).
...
class Book1 {
  ...
  func saveDataToPlist(data: [Book1]? = nil) {
    // if nothing is passed in then save the current book (never do this!)
...
/*:
 * Let's imagine our boss decides to change our persistence to core data, or parse, or both instead of directly saving to a plist.
 ...
 * SRP dictates that we should move the save responsibility to a separate class, because we have 2 distinct axes of change. That is, the Book object could change it's members, it could also change the save functionality.
 */
...
class CoreData: Persistable {
  func save() {
    print("core data saved")
  }
}

class ParseData: Persistable {
  func save() {
    print("parse saved")
  }
}
...
class Book2 {
  ...
  private let dataManager: Persistable
  
  // Notice we're using Dependency Injection. Explain.
  init(title: String, author: String, dataManager: Persistable) {
    self.title = title
    self.author = author
...
class MyVC2: UIViewController {
  func createBook(with title: String, the author: String, and currentPage: Int, dataManager: Persistable) {
