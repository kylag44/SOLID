 ...
 * What we want to avoid is making this "high-level policy" depend on something that is too concrete, such as the Lamp
 */
...
/*:
 * The lamp is now dependent on an abstraction and so is the Button: ButtonServer
 ...
 * We could change the name "ButtonServer" to something more generic, like "SwitchableServer", since not just Button's could use it, any switch could. We could also generalize Button to adopt something like a Switchable interface if we wanted our program to work with other even creators, like motion detectors.
 * It might not help to talk about "inverted dependencies". Just think about the idea that policies in your code that are actually quite abstract, shouldn't have concrete dependencies.
